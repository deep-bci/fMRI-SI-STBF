function [S,par] = STBFSI(B,L,SBFs,varargin)
tic
...
if ~isempty(Phi)
    ...
    for k = 1:K
        ...
        theta(:,k) = repmat(1./gamma,1,nSensor).*F'/Sig_B*residual/x;
...
theta = temp;
  ...
  if strcmpi('Convex', update)
      ...
      Cost(iter) = Cost(iter) - .5*trace(Phi'*temp*Phi) -.5*nSnap*trace(temp*Sigma_C) - .5*nSnap*nSensor*log(2*pi);
  end
%% Check stop conditon
